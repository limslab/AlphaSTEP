...
def concat_proteins(protein_dict: dict, sep="$") -> str:
    ...
    for key in protein_dict:
        ...
        seq_count += len(protein_dict[key]["sequence"]) + 1
    seq_list.append("")
...
class Digest:
    def __init__(
        ...
        protease : str, optional
            ...
            or a regular expression. By default 'trypsin'
...
def get_fix_mods(sequence: str, fix_mod_aas: str, fix_mod_dict: dict):
    """
...
def get_candidate_sites(sequence: str, target_mod_aas: str):
...
# 添加蛋白名和基因名到前体数据框
spec_lib.append_protein_name()

# 保存生成的光谱库到 HDF 文件
output_hdf_file = 'output_spectral_library1.hdf'
spec_lib.save_hdf(output_hdf_file)